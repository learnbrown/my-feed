...
4. 已完成的主页缓存和后续 Feed ZSET 应该如何建立性能证据。
...
- 主页缓存已补 hit、miss、回源、坏 JSON、60 秒 TTL、不续期和主动失效测试。
...
状态：已完成 MVP。实现和验收结果见 `doc/10 用户主页缓存实现方案.md`。
...
- 不引入 singleflight、分布式锁、延迟双删、MQ、预热和空值缓存；并发旧值依靠 60 秒 TTL 收敛。
